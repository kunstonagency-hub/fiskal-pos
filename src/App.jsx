...
function App() {
  ...
  const handleUploadKdsBanner = async (file) => {
    ...
    try {
      ...
      const { error: dbErr } = await supabase
        .from("stores")
        .update({ kds_banners: updatedBanners })
  ...
  const handleDeleteKdsBanner = async (bannerUrl) => {
    if (
      !window.confirm(
        "¿Seguro que deseas eliminar esta imagen de la pantalla de clientes?",
      )
    )
      return;
    try {
      const updatedBanners = currentStoreKdsBanners.filter(
        (url) => url !== bannerUrl,
      );
      const { error: dbErr } = await supabase
        .from("stores")
  ...
  const [currentStoreTaxInclusive, setCurrentStoreTaxInclusive] =
    useState(false);
  ...
  const handleToggleKrono = async (storeId, currentStatus) => {
    try {
      const { error } = await supabase
        .from("stores")
        .update({ krono_enabled: !currentStatus })
        .eq("id", storeId);
      if (error) throw error;
      fetchAdminStores();
    } catch (error) {
  ...
  const handleSaveFiscalSettings = async (e) => {
    ...
    try {
      ...
      if (error) throw error;
      alert(
        "¡Datos del comercio y configuración de impuestos guardados con éxito!",
      );
    } catch (e) {
  ...
  const [trialAlertData, setTrialAlertData] = useState({
    isTrial: true,
    daysLeft: 10,
    expired: false,
  });
  ...
  const handleOpenModifierModal = (prod) => {
    ...
    if (prod.extras) {
      try {
        availableExtras =
          typeof prod.extras === "string"
            ? JSON.parse(prod.extras)
            : prod.extras;
      } catch (e) {
        availableExtras = [];
      }
  ...
  const handleEnviarWhatsApp = () => {
    ...
    if (!cliente || !cliente.phone || cliente.phone === "EMPTY") {
      alert(
        "El cliente seleccionado no tiene un número de teléfono válido registrado.",
      );
      return;
    }
  ...
  const insertarVariable = (variable) => {
    ...
    if (textarea) {
      ...
      const nuevoTexto =
        textoActual.substring(0, start) + variable + textoActual.substring(end);
      ...
      setPlantillas({
        ...plantillas,
        [plantillaActiva]: textoActual + variable,
      });
    }
  };
  ...
  const [selectedRegisterIdForOpen, setSelectedRegisterIdForOpen] =
    useState("");
  ...
  const [plantillas, setPlantillas] = useState({
    ...
    promocionGeneral:
      "¡Hola {cliente}! Tenemos ofertas especiales hoy en {comercio} con el producto {producto}. ¡Visítanos o escríbenos para más detalles!",
  });
    ...
    const salesChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          const storeIdMatch =
            (payload.new &&
              String(payload.new.store_id) === String(currentStoreId)) ||
            (payload.old &&
              String(payload.old.store_id) === String(currentStoreId));
          ...
          if (storeIdMatch) {
            ...
            if (
              ...
              const orderNumber = orderIdStr.startsWith("local")
                ? "Pendiente"
                : orderIdStr.slice(-4);
    ...
    try {
      ...
      if (storeInfo.is_trial) {
        const trialEnd = new Date(
          storeInfo.trial_end_date || storeInfo.created_at,
        ).getTime();
    ...
    try {
      ...
      if (
        ...
        if (navigator.onLine) {
          ...
          if (storeInfo) {
            console.log(
              "🚨 DATOS CRUDOS DE SUPABASE PARA ESTA TIENDA:",
              storeInfo,
            );
            ...
            if (profile.role !== "super_admin") {
              ...
              if (storeInfo.is_active === false) {
                alert(
                  "⚠️ Este comercio se encuentra suspendido por la administración. Acceso denegado.",
                );
            ...
            if (storeInfo.name) {
              ...
              localStorage.setItem(
                `fiskal_cache_store_type_${activeStoreId}`,
                safeType,
              );
              localStorage.setItem(
                `fiskal_cache_store_country_${activeStoreId}`,
                safeCountry,
              );
              localStorage.setItem(
                `fiskal_cache_krono_enabled_${activeStoreId}`,
                storeInfo.krono_enabled || false,
              );
            }
          }
        } else {
          ...
          const cachedType = localStorage.getItem(
            `fiskal_cache_store_type_${activeStoreId}`,
          );
          ...
          const cachedKrono = localStorage.getItem(
            `fiskal_cache_krono_enabled_${activeStoreId}`,
          );
  ...
  const handlePayVendor = async (vendor) => {
    ...
    if (
      !window.confirm(
        `¿Confirmar que ya le pagaste o le vas a liquidar $${vendor.pending_balance.toFixed(2)} al vendedor ${vendor.name}?`,
      )
    )
      return;
    ...
    try {
      ...
      alert(
        `¡Pago de $${vendor.pending_balance.toFixed(2)} registrado exitosamente a ${vendor.name}!`,
      );
      fetchSystemVendors();
      fetchSaasTransactions();
    } catch (e) {
    ...
    try {
      ...
      if (vendorPaidAdvance && newStore) {
        await supabase.from("saas_transactions").insert([
          {
            ...
            amount: priceToLock,
            description:
              "Registro Inicial (Adelanto 1er Mes): " + newStore.name,
        ...
        if (vendorId && vendorRec) {
          const commissionAmount = priceToLock * 0.5;
          await supabase
            .from("system_vendors")
            .update({
              pending_balance:
                parseFloat(vendorRec.pending_balance || 0) + commissionAmount,
              total_earned:
                parseFloat(vendorRec.total_earned || 0) + commissionAmount,
            })
            .eq("id", vendorId);
        }

        alert(
          "¡Comercio registrado exitosamente con 40 DÍAS ACTIVOS!\n\nSe ha generado el 50% de comisión por tu venta inicial.",
        );
      } else {
        alert(
          "¡Comercio registrado exitosamente con 10 días de prueba gratuita y asignado a tu cuenta!",
        );
      }
  ...
  const abrirWhatsAppModal = (
    producto = null,
    clienteNombre = "",
    templateKey = "reposicionStock",
  ) => {
  ...
  const sendWhatsAppReminder = (sale) => {
    ...
    const phone = clientData?.phone
      ? formatWhatsAppNumber(clientData.phone)
      : "";
  ...
  const sendStoreRenewalWhatsApp = (store) => {
    ...
    const finalPrice = getCalculatedMonthlyPrice(
      store.custom_discount,
      store.monthly_price_agreed,
    ).toFixed(2);
  ...
  const fetchAdminStores = async () => {
    try {
      const { data, error } = await supabase
        .from("stores")
        .select("*, system_vendors!system_vendor_id(name)")
        .order("created_at", { ascending: false });
      if (error) throw error;
      setAdminStores(data || []);
    } catch (error) {
        ...
        if (error || !data || data.length === 0) {
          ...
          if (!checkExist || checkExist.length === 0) {
            const { data: newReg, error: insErr } = await supabase
              .from("cash_registers")
              .insert([
                { name: "Caja Principal", is_main: true, store_id: storeId },
              ])
              .select()
              .single();
            if (!insErr && newReg) {
  ...
  const handleDeleteStore = async (storeId, storeName) => {
    if (currentUserRole !== "super_admin") {
      alert(
        "Acceso denegado: Solo el Administrador Principal puede eliminar comercios.",
      );
      return;
    }
    ...
    try {
      ...
      if (error) {
        if (
          ...
          throw new Error(
            "No puedes eliminar este comercio porque tiene productos, cajas o ventas registradas. Debes eliminar primero sus registros o activar borrado en cascada (Cascade) en Supabase.",
          );
        }
        throw error;
      }
      ...
      if (!data || data.length === 0) {
        alert(
          "⚠️ La base de datos no permitió borrar el comercio. Esto ocurre porque la política RLS en Supabase no tiene habilitado el permiso de DELETE.",
        );
        return;
      }
  ...
  const handleStartEditStore = (store) => {
    ...
    setStoreCustomDiscount(
      store.custom_discount !== undefined && store.custom_discount !== null
        ? store.custom_discount
        : globalPromoDiscount,
    );
    ...
    try {
      if (editingStore) {
        ...
        if (storePaidAdvance && newStore && !storeIsDemo) {
          await supabase.from("saas_transactions").insert([
            {
              ...
              amount: priceToLock,
              description:
                "Registro inicial Standalone (Suscripción): " + newStore.name,
              store_id: newStore.id,
            },
          ]);
          ...
          alert(
            storeIsDemo
              ? "¡Comercio de prueba (DEMO) registrado! No afectará la contabilidad."
              : "¡Comercio registrado exitosamente con 10 días de prueba!",
          );
        }
      }
  ...
  const handleRenewSubscription = async (store) => {
    ...
    if (
      !window.confirm(
        `¿Confirmar cobro de renovación de $${finalPrice} por 1 mes (30 días) para: ${store.name}?\n\nSi le quedaban días de prueba o de su mes anterior, se le sumarán automáticamente a su nueva fecha de corte.`,
      )
    )
      return;
    ...
    if (store.is_trial) {
      ...
      newExpirationDate.setDate(
        newExpirationDate.getDate() + 30 + trialDaysLeft,
      );
    } else {
      let subDaysLeft = 0;
    ...
    try {
      ...
      if (!data || data.length === 0) {
        alert(
          "⚠️ La base de datos no permitió actualizar la tienda. Asegúrate de ejecutar el SQL en Supabase.",
        );
        return;
      }
      ...
      if (!store.is_demo) {
        try {
          ...
          if (store.system_vendor_id) {
            const commissionAmount = parseFloat(finalPrice) * 0.2;
            ...
            if (vData) {
              await supabase
                ...
                .update({
                  pending_balance:
                    parseFloat(vData.pending_balance || 0) + commissionAmount,
                  total_earned:
                    parseFloat(vData.total_earned || 0) + commissionAmount,
                })
                .eq("id", store.system_vendor_id);
            }
          }
  ...
  const handleSaasWhatsApp = (store) => {
    ...
    const fee = Number(store.monthly_fee || 13.4).toFixed(2);
...
¡Gracias por confiar en Fiskal para la gestión de su negocio!`;
  ...
  const generateCustomSaaSInvoice = async () => {
    ...
    const basePrice =
      store.monthly_price_agreed !== null &&
      store.monthly_price_agreed !== undefined
        ? store.monthly_price_agreed
        : baseMonthlyPrice;
    const sysDiscount = store.custom_discount || 0;
    const finalSubPrice = getCalculatedMonthlyPrice(
      store.custom_discount,
      store.monthly_price_agreed,
    );
    ...
    if (
      window.confirm(
        `El archivo ${fileName} se ha descargado.\n\n¿Deseas abrir WhatsApp Web ahora para enviar un mensaje al cliente?`,
      )
    ) {
      const phone = formatWhatsAppNumber(store.phone);
      if (!phone) {
        alert(
          "El comercio no tiene un teléfono registrado para abrir WhatsApp.",
        );
        return;
      }
  ...
  const fetchProducts = async (storeId) => {
    ...
    try {
      ...
      const deletedIds = actions
        .filter((a) => a.type === "DELETE_PRODUCT")
        .map((a) => a.productId);
      let finalCloudProducts = cloudProducts.filter(
        (p) => !deletedIds.includes(p.id),
      );

      const productUpdateActions = actions.filter(
        (a) => a.type === "UPDATE_PRODUCT",
      );
  ...
  const fetchSales = async (storeId) => {
    ...
    try {
      ...
      const deletedIds = actions
        .filter((a) => a.type === "DELETE_SALE")
        .map((a) => a.saleId);
      let finalCloudSales = cloudSales.filter(
      ...
      finalCloudSales = finalCloudSales.map((s) => {
        ...
        return update
          ? {
              ...
              balance_due_usd: update.newBalanceDue,
              payment_details: update.paymentDetails,
            }
          : s;
      });
  ...
  const fetchClients = async (storeId) => {
    ...
    try {
      ...
      const deletedIds = actions
        .filter((a) => a.type === "DELETE_CLIENT")
        .map((a) => a.clientId);
      const finalCloudClients = cloudClients.filter(
  ...
  const handleOpenShift = async (e) => {
    ...
    if (!isOnline) {
      alert(
        "Debes tener conexión a internet para Aperturar la Caja por seguridad de la base de datos.",
      );
      return;
    }
    ...
    if (isNaN(regId)) {
      if (registers && registers.length > 0) {
        ...
        alert(
          "Error: No tienes ninguna caja física configurada en tu local. Ve a 'Configuración' para registrar una.",
        );
        return;
      }
    }
    ...
    try {
      ...
      if (checkReg) {
        alert(
          "¡Atención! Esta caja física ya se encuentra abierta y siendo operada por otro usuario.",
        );
        return;
      }
  ...
  const notasFinales =
    fondoBsTag + (shiftNotes ? ` | Cierre: ${shiftNotes}` : "");
  ...
  const handleCloseShift = async () => {
    ...
    const floatBs = match
      ? parseFloat(match[1])
      : Number(
          currentShift.opening_float_ves || currentShift.opening_float_bs || 0,
        );
  ...
  const handleUpdateProduct = async (e) => {
    ...
    if (!editingProduct || !name || !price || !currentStoreId) {
      alert(
        "Falta el nombre, el precio, o no hay una tienda/producto seleccionado. No se guardó ningún cambio.",
      );
      return;
    }
    ...
    try {
      ...
      if (!data || data.length === 0) {
        throw new Error(
          "No se encontró ese producto en tu tienda para actualizar — no se guardó ningún cambio. Refresca la página e intenta de nuevo.",
        );
      }
  ...
  const handleDuplicateProduct = (prod) => {
    ...
    setKronoPrice(
      prod.krono_preferential_price
        ? prod.krono_preferential_price.toString()
        : "",
    );
  ...
  const handleStartEditProduct = (prod) => {
    ...
    setKronoPrice(
      prod.krono_preferential_price !== null &&
        prod.krono_preferential_price !== undefined
        ? prod.krono_preferential_price.toString()
        : "",
    );
              ...
              if (html5QrCodeRef.current) {
                html5QrCodeRef.current
                  ...
                  .then(() => {
                    ...
                    if (handleScannedCodeResultRef.current)
                      handleScannedCodeResultRef.current(decodedText);
                  })
                  .catch(() => {
                    setShowCameraScannerModal(false);
                    if (handleScannedCodeResultRef.current)
  ...
  const handleCapturePhotoScan = async (e) => {
    ...
    try {
      if (!window.BarcodeDetector) {
        alert(
          "Tu navegador no soporta la lectura automática de códigos desde imagen. Usa el buscador manual por SKU.",
        );
        stopCameraScanner();
        return;
      }
  ...
  const handleAddClient = async (e, isQuick = false) => {
    ...
    const existingDup = clients.find(
      (c) => c.document && c.document.trim().toLowerCase() === normalizedDoc,
    );
    if (existingDup) {
      alert(
        `⚠️ Cédula Duplicada: La cédula "${clientDoc}" ya está registrada en el sistema a nombre de ${existingDup.name}.`,
      );
      return;
    }
    ...
    try {
      ...
      if (checkDb) {
        alert(
          `⚠️ Ya existe un cliente con la cédula ${clientDoc} (${checkDb.name}).`,
        );
        setLoadingClient(false);
        return;
      }
  ...
  const handleDeleteClient = async (id) => {
    if (!isOnline) {
      if (id && String(id).startsWith("local_")) {
        const actions = await getOfflineActions();
        const action = actions.find(
          (a) => a.type === "INSERT_CLIENT" && a.tempId === id,
        );
        if (action) await clearOfflineAction(action.local_id);
      } else {
  ...
  const amountAlreadyPaid =
    settlingSale && settlingSale.balance_due_usd !== undefined
  ...
  const paidUSDFromCashBs =
    currentStoreCountry === "venezuela"
      ? calcPayments.cashBs / (bcvRate || 1)
      : 0;
  const paidUSDFromPagoMovil =
    currentStoreCountry === "venezuela"
      ? calcPayments.pagoMovil / (bcvRate || 1)
      : 0;
  const paidUSDFromZelle = calcPayments.zelle;
  const paidUSDFromDebit =
    currentStoreCountry === "venezuela"
  ...
  const changeUSD = Math.max(
    0,
    parseFloat((totalPaidUSD - totalUSD).toFixed(2)),
  );
  ...
  const handleResumeOrder = (sale) => {
    if (cart.length > 0) {
      if (
        !window.confirm(
          "Tienes productos en el carrito actual. ¿Deseas reemplazarlos por esta comanda?",
        )
      ) {
        return;
      }
  ...
  const handleCreditCheckout = async () => {
    ...
    if (
      !window.confirm(
        `¿Registrar venta a CRÉDITO para ${selectedClient} por $${totalUSD.toFixed(2)}?`,
      )
    )
      return;
    ...
    if (settlingSale && !String(settlingSale.id).startsWith("local_")) {
      ...
      if (!isOnline) {
        ...
        const { error, data: newSale } = await supabase
          .from("sales")
          .insert([saleData])
          .select()
          .single();
  ...
  const handlePrintZReport = () => {
    ...
    const shiftSales = (typeof sales !== "undefined" ? sales : []).filter(
      (sale) =>
        sale.shift_id === selectedShiftReport.id && sale.status === "completed",
    );
    const emp = (typeof employees !== "undefined" ? employees : []).find(
      (e) => e.id === selectedShiftReport.user_id,
    ) || { full_name: "Cajero" };
    const reg = (typeof registers !== "undefined" ? registers : []).find(
      (r) => r.id === selectedShiftReport.register_id,
    ) || { name: "Caja Principal" };
  ...
  const filteredProductsForCatalog = (products || []).filter((p) => {
    ...
    const searchQuery =
      typeof productSearchQuery !== "undefined" && productSearchQuery
        ? String(productSearchQuery).toLowerCase()
        : "";
    ...
    const matchesQuery =
      ...
      (productBarcode && productBarcode.includes(searchQuery));
  ...
  const currentShiftSales = currentShift
    ? sales.filter(
        (s) => s.shift_id === currentShift.id && s.status === "completed",
      )
    : [];
  const shiftTotalUSD = currentShiftSales.reduce(
    (sum, s) => sum + s.total_usd,
    0,
  );
  const shiftZelle = currentShiftSales.reduce(
    (sum, s) => sum + (s.payment_details?.zelle || 0),
    0,
  );
  const shiftPagoMovilBs = currentShiftSales.reduce(
    (sum, s) => sum + (s.payment_details?.pago_movil || 0),
    0,
  );
  const shiftDebitBs = currentShiftSales.reduce(
    (sum, s) => sum + (s.payment_details?.debit || 0),
    0,
  );
  ...
  const shiftCashUSD = currentShiftSales.reduce((sum, s) => {
    ...
    const cashIn = Number(
      pd.raw_cash_usd !== undefined ? pd.raw_cash_usd : pd.cash_usd || 0,
    );
    const changeOut =
      !pd.change_currency_type || pd.change_currency_type === "USD"
        ? Number(pd.change_usd || 0)
        : 0;
  ...
  const shiftCashBs = currentShiftSales.reduce((sum, s) => {
    const pd = s.payment_details || {};
    const cashIn = Number(
      pd.raw_cash_bs !== undefined ? pd.raw_cash_bs : pd.cash_bs || 0,
    );
  ...
  const shiftChangePagoMovilBs = currentShiftSales.reduce((sum, s) => {
    ...
    return (
      sum +
      (pd.change_currency_type === "PAGO_MOVIL" ? Number(pd.change_bs || 0) : 0)
    );
  }, 0);
  ...
  const shiftChangePagoMovilUSD = currentShiftSales.reduce((sum, s) => {
    ...
    return (
      sum +
      (pd.change_currency_type === "PAGO_MOVIL"
        ? Number(pd.change_usd || 0)
        : 0)
    );
  }, 0);
  ...
  const getFilteredClientsByTab = () => {
    if (clientFilterTab === "best") {
      return [...clientsWithMetrics].sort(
  ...
  const getClientHistoryAndTopProducts = (clientName) => {
    const cliSales = sales.filter(
      (s) => s.client_name === clientName && s.status !== "pending",
    );
    const prodCounts = {};
  ...
  const obtenerProductosMasVendidos = () => {
    ...
    sales.forEach((venta) => {
      ...
      venta.items?.forEach((item) => {
        ...
        conteo[item.id].compradores[clienteNombre] =
          (conteo[item.id].compradores[clienteNombre] || 0) + item.quantity;
      });
    });
  ...
  const getSystemFinancials = () => {
    ...
    const totalPendingComm = systemVendors.reduce(
      (sum, v) => sum + (parseFloat(v.pending_balance) || 0),
      0,
    );
  ...
  if (isRecoveringPassword) {
    return (
      <div
        className="fiskal-login-container"
        style={{
          display: "flex",
      ...
      >
        ...
        >
          ...
          >
            Ingresa tu nueva contraseña para acceder a Fiskal.
          </p>
  ...
  if (!session) {
    return (
      ...
      >
        ...
        >
          <div style={{ textAlign: "center", marginBottom: "24px" }}>
            ...
            <p style={{ fontSize: "13px", color: "#6c757d", margin: 0 }}>
              Sistema de Gestión Comercial y POS
            </p>
          </div>
              ...
              >
                ...
                {resetMessage && (
                  <p
                    style={{
                      fontSize: "11px",
                      color: resetMessage.includes("Error")
                        ? "#c92a2a"
                        : "#2b8a3e",
                      marginBottom: "8px",
                      lineHeight: "1.4",
        ...
        >
          © 2026 Fiskal. Desarrollado por{" "}
          <a
            href="https://wa.me/50764749094?text=Hola,%20me%20gustaría%20más%20información%20sobre%20sus%20servicios."
            target="_blank"
            rel="noopener noreferrer"
            style={{
              color: "#1c7ed6",
  ...
  return (
    <div className="fiskal-container">
      ...
      >
        ...
        <nav className="nav-menu">
          ...
          {currentUserRole === "system_vendor" && (
            <button
              className={
                activeTab === "vendor_portal" ? "nav-btn active" : "nav-btn"
              }
              onClick={(e) => {
                e.stopPropagation();
              ...
              style={{ color: "#2b8a3e", fontWeight: "bold" }}
            >
                    ...
                    : activeTab === "clients"
                      ...
                      : activeTab === "delivery"
                        ...
                        : activeTab === "vendor_portal"
                          ? "Portal de Vendedor de Sistema (Alta de Comercios)"
                          : activeTab === "admin"
                            ? "Panel Maestro SaaS (Administración)"
                            : activeTab === "settings"
          ...
          >
            ...
            >
              ...
              {(() => {
                ...
                availableExtras.forEach((ex) => {
                  if (selectedExtrasToggles[ex.name])
                    extrasSum += parseFloat(ex.price) || 0;
                });
                ...
                return (
                  ...
                  >
                    ...
                    >
                      Precio Base: ${productForModifiers.price.toFixed(2)}{" "}
                      {extrasSum > 0 && `(Extras: +$${extrasSum.toFixed(2)})`}
                    </span>
                    <strong
                      style={{
                        fontSize: "18px",
            ...
            >
              ...
              <button
                type="button"
                className="btn-primary"
                onClick={confirmAddToCartWithModifiers}
                style={{
                  background: "#111827",
                  color: "#ffffff",
                  border: "none",
        ...
        <div className="modal-overlay" style={{ zIndex: 10007 }}>
          ...
          >
            ...
            >
              ...
              >
                Precio base:{" "}
                <strong>${productForWeight.price.toFixed(2)} USD</strong> por
                cada 1{" "}
                {productForWeight.modifiers && productForWeight.modifiers[0]
                  ? productForWeight.modifiers[0]
                  : "kg"}
                .
              </p>
              ...
              <div className="form-group">
                <label>
                  Cantidad en la Balanza (
                  {weightUnit === "kg" ? "Kilogramos" : "Gramos"})
                </label>
              ...
              >
                ...
                >
                  ...
                  {(
                    ...
                    (weightUnit === "g"
                      ? productForWeight.price / 1000
                      : productForWeight.price)
                  ).toFixed(2)}{" "}
                  USD
                </span>
              </div>
            ...
            >
              ...
              <button
                type="button"
                className="btn-primary"
                onClick={confirmAddToCartWithWeight}
                  ...
                  return (
                    <>
                      ...
                      {(!currentStoreCountry ||
                        ...
                        >
                          Bs.{" "}
                          {(floatBs + shiftCashBs).toLocaleString("es-VE", {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                          })}
                        </h3>
          ...
          >
            <div style={{ padding: "20px", overflowY: "auto" }}>
              <div style={{ textAlign: "center", marginBottom: "20px" }}>
                ...
                >
                  Detalle de Reporte Z (Auditoría)
                </h3>
                <p
                  style={{
                    margin: "2px 0",
                ...
                >
                  ...
                  {new Date(selectedShiftReport.opened_at).toLocaleString()}
                </p>
                <p
                  style={{
                    margin: "2px 0",
                ...
                >
                  ...
                  {selectedShiftReport.closed_at
                    ? new Date(selectedShiftReport.closed_at).toLocaleString()
                    : "Turno Abierto"}
                </p>
                <p
                  style={{
                ...
                >
                  ...
                  {employees.find((e) => e.id === selectedShiftReport.user_id)
                    ?.full_name || "Cajero"}
                </p>
              </div>
              ...
              <div style={{ marginBottom: "24px" }}>
                ...
                >
                  Desglose de Ingresos Calculados:
                </h4>
                {(() => {
                  const shiftSales = (
                    ...
                    (sale) =>
                      sale.shift_id === selectedShiftReport.id &&
                  ...
                  const floatUsd = Number(
                    selectedShiftReport?.opening_float_usd ||
                      selectedShiftReport?.opening_float ||
                      0,
                  );
                  const match =
                    selectedShiftReport?.notes?.match(/FondoBs:([0-9.]+)/);
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <strong>${floatUsd.toFixed(2)}</strong>
                      </div>
                      {currentStoreCountry === "venezuela" && (
                        <div
                          style={{
                      ...
                      >
                        <span>Zelle:</span>{" "}
                        <strong>${tZelle.toFixed(2)}</strong>
                      </div>
                      <div
                        style={{
                          display: "flex",
              ...
              <div>
                ...
                >
                  Detalle de Facturas y Productos Vendidos:
                </h4>
                {(() => {
                  ...
                  return shiftSalesList.map((sale, index) => (
                    ...
                    >
                      ...
                      >
                        <strong style={{ fontSize: "13px", color: "#1971c2" }}>
                          Factura #{sale.receipt_number || `A-00${index + 1}`} -{" "}
                          {sale.client_name || "Cliente General"}
                        </strong>
                        <strong style={{ fontSize: "13px", color: "#1971c2" }}>
                          ${(sale.total_usd || 0).toFixed(2)} USD
                        </strong>
                      </div>
            ...
            >
              ...
              >
                <div>
                  ...
                  <br />
                  <strong>
                    {selectedClientDetail.document || "No registrada"}
                  </strong>
                </div>
                <div>
                  <span>Teléfono:</span>
                  <br />
                  <strong>
                    {selectedClientDetail.phone || "No registrado"}
                  </strong>
                </div>
                <div>
                  ...
                  <strong style={{ color: "#2b8a3e" }}>
                    ${selectedClientDetail.totalBilled.toFixed(2)}
                  </strong>
                </div>
                <div>
                  ...
                  <strong
                    style={{
                      color:
                        selectedClientDetail.totalPending > 0
                          ? "#fa5252"
                          : "#2b8a3e",
                    }}
                  >
                    ${selectedClientDetail.totalPending.toFixed(2)}
                  </strong>
                </div>
              </div>
              ...
              <div>
                ...
                >
                  <table className="fiskal-table" style={{ fontSize: "12px" }}>
                    ...
                    <tbody>
                      {getClientHistoryAndTopProducts(selectedClientDetail.name)
                        ...
                        getClientHistoryAndTopProducts(
                      ...
                      {getClientHistoryAndTopProducts(selectedClientDetail.name)
                        ...
                        getClientHistoryAndTopProducts(
                          ...
                          <tr key={s.id}>
                            ...
                            <td>
                              <strong>${s.total_usd.toFixed(2)}</strong>
                            </td>
          ...
          <div className="modal-content" style={{ width: "560px" }}>
            ...
            >
              <div className="form-group">
                ...
                <textarea
                  rows="4"
                  value={mensajePersonalizadoTemp}
                  onChange={(e) => setMensajePersonalizadoTemp(e.target.value)}
                  style={{
                    width: "100%",
                    padding: "10px",
                    borderRadius: "6px",
            ...
            >
              ...
              <button
                type="button"
                className="btn-primary"
                onClick={enviarMensajeWhatsAppFinal}
                style={{ background: "#2b8a3e" }}
              >
                Abrir WhatsApp
              </button>
        ...
        <div className="modal-overlay" style={{ zIndex: 10000 }}>
          <div className="modal-content" style={{ width: "500px" }}>
            <div className="modal-header">
              <h3>
                Factura #
                {String(selectedInvoice.id).startsWith("local")
                  ? "Pendiente"
                  : selectedInvoice.invoice_number ||
                    `A-${String(selectedInvoice.id).padStart(3, "0")}`}
              </h3>
              <button
                className="btn-close-modal"
            ...
            >
              ...
              >
                <span>
                  ...
                  {selectedInvoice.client_name || "Cliente General"}
                </span>
                <span>
                  <strong>
                    {currentStoreCountry === "venezuela"
                  ...
                  {selectedInvoice.payment_details?.client_document || "N/A"}
                </span>
              </div>
              <div
                style={{
              ...
              {(() => {
                const isVzla = currentStoreCountry === "venezuela";
                const saleBcvRate =
                  selectedInvoice.payment_details?.applied_bcv_rate ||
                  bcvRate ||
                  1;
                  ...
                  <>
                    {isVzla && (
                      ...
                      >
                        ...
                        {saleBcvRate.toLocaleString("es-VE", {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </div>
                        ...
                        <tbody>
                          {(() => {
                            ...
                            if (Array.isArray(selectedInvoice.items)) {
                              ...
                              try {
                                parsedItems = JSON.parse(selectedInvoice.items);
                              } catch (e) {}
                            }
                    ...
                    >
                        ...
                        <>
                          ...
                          >
                            ...
                            <div style={{ textAlign: "right" }}>
                              <strong>
                                {formatMoney(
                                  selectedInvoice.subtotal_usd ||
                                    selectedInvoice.total_usd -
                                      selectedInvoice.tax_usd,
                                )}
                              </strong>
                              <div
                                style={{
                          ...
                          >
                            ...
                            <div style={{ textAlign: "right" }}>
                              <strong>
                                {formatMoney(selectedInvoice.tax_usd)}
                              </strong>
                              <div
                                style={{
                                  fontSize: "11px",
                              ...
                              >
                                {formatRef(selectedInvoice.tax_usd)}
                              </div>
                            </div>
                          </div>
                        </>
                      ...
                      >
                        ...
                        <div style={{ textAlign: "right" }}>
                          <strong>
                            {formatMoney(selectedInvoice.total_usd)}
                          </strong>
                          <div
                            style={{
                              fontSize: "12px",
                          ...
                          >
                            {formatRef(selectedInvoice.total_usd)}
                          </div>
                        </div>
                      </div>
                      ...
                      {selectedInvoice.balance_due_usd > 0 && (
                        ...
                        >
                          ...
                          <div style={{ textAlign: "right" }}>
                            <strong>
                              {formatMoney(selectedInvoice.balance_due_usd)}
                            </strong>
                            <div
                              style={{ fontSize: "11px", fontWeight: "normal" }}
                            >
                              {formatRef(selectedInvoice.balance_due_usd)}
                            </div>
                          </div>
                        </div>
                      )}
                    ...
                    {selectedInvoice.payment_details && (
                      ...
                      >
                        ...
                        {selectedInvoice.payment_details.raw_cash_usd > 0 && (
                          ...
                          >
                            <span style={{ color: "#495057" }}>
                              Efectivo USD Recibido:
                            </span>
                            <strong>
                              $
                              {selectedInvoice.payment_details.raw_cash_usd.toFixed(
                                2,
                              )}
                            </strong>
                          </div>
                        ...
                        {selectedInvoice.payment_details.raw_cash_bs > 0 && (
                          ...
                          >
                            <span style={{ color: "#495057" }}>
                              Efectivo Bs Recibido:
                            </span>
                            <strong>
                              Bs.{" "}
                              {selectedInvoice.payment_details.raw_cash_bs.toLocaleString(
                                "es-VE",
                                { minimumFractionDigits: 2 },
                              )}
                            </strong>
                        ...
                        {selectedInvoice.payment_details.pago_movil > 0 && (
                          ...
                          >
                            ...
                            <strong>
                              Bs.{" "}
                              {selectedInvoice.payment_details.pago_movil.toLocaleString(
                                "es-VE",
                                { minimumFractionDigits: 2 },
                              )}
                            </strong>
                        ...
                        {selectedInvoice.payment_details.zelle > 0 && (
                          ...
                          >
                            ...
                            <strong>
                              $
                              {selectedInvoice.payment_details.zelle.toFixed(2)}
                            </strong>
                          </div>
                        )}
                        {selectedInvoice.payment_details.debit > 0 && (
                          ...
                          >
                            ...
                            <strong>
                              {isVzla
                                ? `Bs. ${selectedInvoice.payment_details.debit.toLocaleString("es-VE", { minimumFractionDigits: 2 })}`
                                : `$${selectedInvoice.payment_details.debit.toFixed(2)}`}
                            </strong>
                          </div>
                        )}

                        {(selectedInvoice.payment_details.change_usd || 0) >
                          0 && (
                          <div
                            style={{
                              display: "flex",
                          ...
                          >
                            ...
                            <span>
                              $
                              {selectedInvoice.payment_details.change_usd.toFixed(
                                2,
                              )}
                              {selectedInvoice.payment_details
                                .change_currency_type === "BS"
                                ? " (En Efectivo Bs)"
                                : selectedInvoice.payment_details
                                      .change_currency_type === "PAGO_MOVIL"
                                  ? " (Por Pago Móvil)"
                                  : " (En Efectivo USD)"}
                            </span>
                          </div>
              ...
              {invoiceHistory && invoiceHistory.length > 0 && (
                <div style={{ marginTop: "16px" }}>
                  ...
                  {invoiceHistory.map((h, i) => {
                    const histBcvRate =
                      h.payment_details?.applied_bcv_rate ||
                      selectedInvoice.payment_details?.applied_bcv_rate ||
                      bcvRate ||
                      1;
      ...
      {showLabelModal && labelProduct && (
        <div className="modal-overlay" style={{ zIndex: 10005 }}>
          <div
            className="modal-content label-modal-content"
            style={{ width: "380px" }}
          >
            <div className="modal-header">
              <h3 style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <div
              className="modal-body label-print-area"
              style={{ textAlign: "center", padding: "24px" }}
            >
              <div className="store-tag-header">
                {currentStoreName.toUpperCase()}
              </div>
              ...
              <div className="tag-price-box">
                ...
                <span className="tag-price-value">
                  ${labelProduct.price.toFixed(2)}
                </span>
              </div>
              <div
                style={{ fontSize: "11px", color: "#6c757d", marginTop: "6px" }}
              >
      ...
      {showPrintCatalog && (
        <div className="modal-overlay" style={{ zIndex: 10001 }}>
          <div
            className="modal-content letter-print"
            style={{
              width: "800px",
              maxWidth: "95%",
              maxHeight: "90vh",
          ...
          >
            <div className="modal-header">
              ...
              <button
                className="btn-close-modal"
                onClick={() => setShowPrintCatalog(false)}
              >
                <X size={20} />
              </button>
            </div>
            <div className="modal-body" style={{ background: "#f8f9fa" }}>
              <div className="catalog-print-grid">
                {products.length === 0 ? (
                  ...
                  >
                    No hay productos registrados para imprimir.
                  </p>
                ) : (
                  products.map((prod) => (
                    <div key={prod.id} className="print-label-item">
                      ...
                      />
                      <div
                        className="tag-price-box"
                        style={{ padding: "4px 12px", marginTop: "4px" }}
                      >
                        <span
                          className="tag-currency"
                          style={{ fontSize: "10px" }}
                        >
                          USD
                        </span>
                        <span
                          className="tag-price-value"
                          style={{ fontSize: "16px" }}
                        >
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  <div>
                    <strong>Comercio:</strong> {preInvoiceStore.name}
                  </div>
                  <div>
                    <strong>Propietario:</strong>{" "}
                    {preInvoiceStore.owner_name ||
                      preInvoiceStore.full_name ||
                      "Nombres"}
                  </div>
                  <div>
                    <strong>
                    ...
                    {preInvoiceStore.rif || preInvoiceStore.document || "N/A"}
                  </div>
                  <div>
                ...
                >
                  <thead>
                    ...
                    >
                      <th style={{ padding: "10px 5px", fontWeight: "bold" }}>
                        Descripción
                      </th>
                      <th
                  ...
                  <tbody>
                    <tr style={{ borderBottom: "1px solid #eee" }}>
                      ...
                      >
                        $
                        {(preInvoiceStore.monthly_price_agreed !== null &&
                        preInvoiceStore.monthly_price_agreed !== undefined
                          ? preInvoiceStore.monthly_price_agreed
                          : baseMonthlyPrice
                        ).toFixed(2)}
                      </td>
                      <td
                        ...
                        style={{ padding: "15px 5px", textAlign: "center" }}
                      >
                        {preInvoiceStore.custom_discount > 0
                          ? `${preInvoiceStore.custom_discount}%`
                          : "0%"}
                      </td>
                      <td
                        className="text-right"
                        style={{ padding: "15px 5px", textAlign: "right" }}
                      >
                        $
            ...
            <form onSubmit={handleCreateStoreOwnerSubmit}>
              <div className="modal-body fiskal-form">
                ...
                <div className="form-group">
                  ...
                  <input
                    type="text"
                    value={ownerModalName}
                    onChange={(e) => setOwnerModalName(e.target.value)}
                    required
                  />
                </div>
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <input
                    type="text"
                    value={ownerModalPass}
                    onChange={(e) => setOwnerModalPass(e.target.value)}
                    required
                    minLength={6}
                    placeholder="Mínimo 6 caracteres"
                  />
              ...
              >
                ...
                <button
                  type="submit"
                  className="btn-primary"
                  disabled={creatingOwnerLoading}
                >
                  {creatingOwnerLoading
                    ? "Creando..."
                    : "Crear Cuenta y Asignar"}
                </button>
              </div>
            </form>
          </div>
