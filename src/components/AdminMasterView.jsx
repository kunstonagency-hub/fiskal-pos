...
// Vista maestra de administración.
// Centraliza el control financiero, gestión de tiendas, vendedores, pagos SaaS y
// operaciones administrativas del sistema para el super administrador.
function AdminMasterView({
  ...
  const handleConfirmSaaSPayment = async (e) => {
    ...
    if (
      (renewPaymentMethod === "pago_movil" || renewPaymentMethod === "zelle") &&
      !renewReference.trim()
    ) {
      alert(
        "Por favor ingresa el número de referencia bancaria para tu control.",
      );
      return;
    }
      ...
      if (!store.is_demo) {
        try {
          ...
          if (store.system_vendor_id && !isPendingPayment) {
            const commissionAmount = finalPriceNum * 0.2;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      >
        <div
          ...
          style={{ padding: "20px", borderLeft: "4px solid #1c7ed6" }}
        >
          <div
            style={{
              display: "flex",
          ...
          >
            <span
              style={{ fontSize: "13px", color: "#6c757d", fontWeight: "bold" }}
            >
              Ingresos Cobrados (Suscripciones)
            </span>
            <Activity size={18} color="#1c7ed6" />
          </div>
          <h2 style={{ fontSize: "28px", marginTop: "12px", color: "#212529" }}>
            ${getSystemFinancials().totalIncome.toFixed(2)}
          </h2>
        </div>
        <div
          className="product-form-card"
          style={{ padding: "20px", borderLeft: "4px solid #fa5252" }}
        >
          <div
            style={{
              display: "flex",
          ...
          <h2 style={{ fontSize: "28px", marginTop: "12px", color: "#212529" }}>
            ${getSystemFinancials().totalExpenses.toFixed(2)}
          </h2>
        </div>
        <div
          className="product-form-card"
          style={{ padding: "20px", borderLeft: "4px solid #f59f00" }}
        >
          <div
            style={{
              display: "flex",
          ...
          >
            <span
              style={{ fontSize: "13px", color: "#6c757d", fontWeight: "bold" }}
            >
              Comisiones por Liquidar (Pendiente)
            </span>
            <Clock size={18} color="#f59f00" />
          </div>
          <h2 style={{ fontSize: "28px", marginTop: "12px", color: "#212529" }}>
            ${getSystemFinancials().totalPendingComm.toFixed(2)}
          </h2>
        </div>
        <div
          className="product-form-card"
        ...
        >
          ...
          >
            <span
              style={{ fontSize: "13px", color: "#2b8a3e", fontWeight: "bold" }}
            >
              Beneficio Neto del Sistema
            </span>
            <TrendingUp size={18} color="#2b8a3e" />
          </div>
          <h2 style={{ fontSize: "28px", marginTop: "12px", color: "#2b8a3e" }}>
            ${getSystemFinancials().netProfit.toFixed(2)}
          </h2>
        </div>
      </div>
      ...
      >
        <div className="product-form-card" style={{ background: "#f8f9fa" }}>
          ...
          >
            Ajusta la tarifa base del sistema. Quienes se registren hoy quedarán
            atados permanentemente a esta tarifa, incluso si la subes en el
            futuro.
          </p>
          <form onSubmit={handleSaveSaasSettings} className="fiskal-form">
            ...
            <div className="form-group">
              ...
              <span style={{ fontSize: "11px", color: "#fa5252" }}>
                {globalPromoDiscount > 0
                  ? `Un nuevo registro hoy pagará $${getCalculatedMonthlyPrice(0, baseMonthlyPrice).toFixed(2)} /mes de por vida.`
            ...
            >
              {savingSettings
                ? "Guardando..."
                : "Aplicar Precios a Nuevos Registros"}
            </button>
          </form>
        </div>
        ...
        <div className="product-list-card">
            ...
            <table className="fiskal-table" style={{ fontSize: "13px" }}>
              ...
              <tbody>
                {systemVendors.length === 0 ? (
                  ...
                  systemVendors.map((v) => {
                    const vendorStores = adminStores.filter(
                      (s) => s.system_vendor_id === v.id,
                    );
                    const activeCount = vendorStores.filter(
                      (s) => s.is_active,
                    ).length;
                    return (
                      <tr key={v.id}>
                        ...
                        <td style={{ textAlign: "center" }}>
                          <span className="badge-completed">
                            {activeCount} / {vendorStores.length}
                          </span>
                        </td>
                        <td>
                          <strong>
                            ${(parseFloat(v.total_earned) || 0).toFixed(2)}
                          </strong>
                        </td>
                        <td>
                          <strong
                            style={{
                              color:
                                (parseFloat(v.pending_balance) || 0) > 0
                                  ? "#d9480f"
                                  : "#2b8a3e",
                              fontSize: "14px",
                            }}
                          >
                            ${(parseFloat(v.pending_balance) || 0).toFixed(2)}
                          </strong>
                        </td>
                        <td style={{ textAlign: "center" }}>
                          <button
          ...
          <p style={{ fontSize: "11px", color: "#6c757d", marginTop: "12px" }}>
            * El saldo pendiente suma automáticamente el 50% de la cuota de
            nuevos registros y el 20% recurrente de sus renovaciones mensuales.
          </p>
        </div>
      </div>
      ...
      <div className="products-layout">
        <div className="product-form-card">
          <h3>
            {editingStore
              ? `Editando: ${editingStore.name}`
              : "Registrar Nuevo Comercio SaaS"}
          </h3>
          <form onSubmit={handleSaveStore} className="fiskal-form">
            ...
            {editingStore && (
              ...
              >
                ...
                <input
                  ...
                  max="100"
                  min="0"
                  value={storeCustomDiscount}
                  onChange={(e) => setStoreCustomDiscount(e.target.value)}
                />
                <span
                  style={{
                    fontSize: "11px",
                ...
                >
                  Este comercio tiene un precio base congelado de{" "}
                  <strong>
                    ${editingStore.monthly_price_agreed || baseMonthlyPrice}
                  </strong>
                  . Con el {storeCustomDiscount}% de descuento pasará a pagar{" "}
                  <strong>
                    $
                    {getCalculatedMonthlyPrice(
                      storeCustomDiscount,
                      editingStore.monthly_price_agreed,
                    ).toFixed(2)}
                  </strong>{" "}
                  mensuales.
                </span>
            ...
            {!editingStore && (
              ...
              >
                <input
                  type="checkbox"
                  id="storePaidAdvance"
                  checked={storePaidAdvance}
                  onChange={(e) => setStorePaidAdvance(e.target.checked)}
                  style={{ width: "18px", height: "18px", cursor: "pointer" }}
                />
                <label
                  htmlFor="storePaidAdvance"
                  style={{
                    cursor: "pointer",
                    fontSize: "13px",
                    fontWeight: "bold",
                ...
                >
                  ¿El comercio pagó el mes por adelantado? (Activa 40 días: 30
                  de mes + 10 de cortesía)
                </label>
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: "8px", marginTop: "8px" }}>
              ...
              <button type="submit" className="btn-primary" style={{ flex: 2 }}>
                <Store size={18} />{" "}
                {editingStore ? "Actualizar Comercio" : "Registrar Comercio"}
              </button>
            </div>
          </form>
        </div>
        ...
        <div className="product-form-card">
          ...
          <form onSubmit={handleCreateSystemVendor} className="fiskal-form">
            ...
            <div className="form-group">
              ...
              <input
                ...
                placeholder="vendedor@fiskal.com"
                required
              />
            </div>
            <div className="form-group">
            ...
            >
              <UserPlus size={18} />{" "}
              {creatingVendor ? "Creando..." : "Crear Vendedor de Sistema"}
            </button>
          </form>
          ...
          <div style={{ marginTop: "20px" }}>
            ...
            >
              Vendedores Activos ({systemVendors.length})
            </h4>
              ...
              {adminStores.length === 0 ? (
                ...
                adminStores.map((store) => {
                  ...
                  if (store.is_trial && store.trial_end_date) {
                    ...
                    daysText =
                      d > 0 ? `${d} días de prueba` : "Prueba expirada";
                    isExpiringSoon = d <= 3;
                  } else if (store.subscription_expires_at) {
                    const diff =
                      new Date(store.subscription_expires_at).getTime() - now;
                    const d = Math.ceil(diff / (1000 * 60 * 60 * 24));
                    daysText =
                      d > 0 ? `${d} días de mes activo` : "Suscripción vencida";
                    isExpiringSoon = d <= 5;
                  }
                  ...
                  const finalDisplayPrice = getCalculatedMonthlyPrice(
                    store.custom_discount,
                    store.monthly_price_agreed,
                  );
                  ...
                  return (
                    <tr key={store.id}>
                      <td>
                        ...
                        <span style={{ fontSize: "11px", color: "#d9480f" }}>
                          Vendedor:{" "}
                          <strong>
                            {store.system_vendors?.name || "Admin Central"}
                          </strong>
                        </span>
                        <br />
                        <span
                      ...
                      <td>
                        <strong>${finalDisplayPrice.toFixed(2)}</strong>
                        <br />
                        {hasCustomDisc && (
                          <span
                            style={{
                          ...
                          >
                            -{store.custom_discount}% aplicado
                          </span>
                        )}
                      </td>
                      <td>
                        <span
                          style={{
                            fontSize: "12px",
                            fontWeight: "bold",
                            color: isExpiringSoon ? "#fa5252" : "#2b8a3e",
                          }}
                        >
                          {daysText}
                        </span>
                      ...
                      <td>
                        {!store.is_active ? (
                          ...
                          >
                            <AlertCircle size={12} /> Suspendido
                          </span>
                        ) : daysText.includes("expirada") ||
                          daysText.includes("vencida") ? (
                          <span
                            className="badge-credit"
                            style={{ background: "#fff3bf", color: "#d9480f" }}
                          >
                      ...
                      <td className="action-cell">
                        ...
                        >
                          <button
                            ...
                            onClick={() => sendStoreRenewalWhatsApp(store)}
                            title="Enviar WhatsApp de Renovación / Cobro"
                          >
                          ...
                          <button
                            ...
                            onClick={() => handleOpenPreInvoice(store)}
                            title="Generar Recibo / Factura SaaS"
                            style={{
                              background: "#4c6ef5",
                              color: "#fff",
                              border: "none",
                          ...
                          <button
                            ...
                            title="Editar Datos y Promociones"
                          >
                            <Edit2 size={16} />
                          </button>
                          ...
                          <button
                            className="btn-secondary"
                            onClick={() =>
                              handleToggleDemo(store.id, store.is_demo)
                            }
                            style={{
                              borderColor: store.is_demo
                                ? "#fa5252"
                                : "#ced4da",
                            ...
                            title={
                              store.is_demo
                                ? "Comercio marcado como DEMO"
                                : "Comercio marcado como REAL"
                            }
                          >
                          ...
                          <button
                            className="btn-secondary"
                            onClick={() =>
                              handleToggleKrono(store.id, store.krono_enabled)
                            }
                            style={{
                              borderColor: store.krono_enabled
                                ? "#10b981"
                                : "#ced4da",
                              color: store.krono_enabled
                                ? "#10b981"
                                : "#6c757d",
                              background: store.krono_enabled
                                ? "#ecfdf5"
                                : "#f8f9fa",
                              fontSize: "11px",
                              padding: "4px 8px",
                            ...
                            title={
                              store.krono_enabled
                                ? "Krono Market está ACTIVO"
                                : "Activar Krono Market"
                            }
                          >
                          ...
                          <button
                            ...
                            style={{
                              borderColor: store.is_active
                                ? "#fa5252"
                                : "#2b8a3e",
                              color: store.is_active ? "#fa5252" : "#2b8a3e",
                              fontSize: "11px",
                              padding: "4px 8px",
                            }}
                          >
      ...
      {saasTransactions && saasTransactions.length > 0 && (
        <div className="product-list-card">
          ...
          >
            <FileText size={18} /> Historial de Cobros y Transacciones del
            Sistema
          </h3>
          <div className="table-responsive">
            <table className="fiskal-table" style={{ fontSize: "13px" }}>
                  ...
                  <tr key={t.id}>
                    ...
                    <td>
                      <span
                        style={{
                          background:
                            t.payment_method === "pago_movil"
                              ? "#e7f5ff"
                              : t.payment_method === "zelle"
                                ? "#f3d9fa"
            ...
            <form onSubmit={handleConfirmSaaSPayment}>
              <div className="modal-body fiskal-form">
                ...
                >
                  ...
                  >
                    Dueño:{" "}
                    <strong>{targetStoreForRenew.owner_name || "N/A"}</strong> |
                    Tel: {targetStoreForRenew.phone || "N/A"}
                  </div>
                </div>
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: "6px",
                        border:
                          renewPaymentMethod === "zelle"
                            ? "2px solid #862e9c"
                            : "1px solid #ced4da",
                        background:
                          renewPaymentMethod === "zelle" ? "#f3d9fa" : "#fff",
                        color:
                          renewPaymentMethod === "zelle"
                            ? "#862e9c"
                            : "#495057",
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: "6px",
                        border:
                          renewPaymentMethod === "pending"
                            ? "2px solid #d9480f"
                            : "1px solid #ced4da",
                        background:
                          renewPaymentMethod === "pending" ? "#fff3bf" : "#fff",
                        color:
                          renewPaymentMethod === "pending"
                            ? "#d9480f"
                            : "#495057",
                ...
                {renewPaymentMethod === "pago_movil" && (
                  ...
                  >
                    <span style={{ color: "#1864ab" }}>
                      Total recibido en Bs (Tasa{" "}
                      {bcvRate ? bcvRate.toFixed(2) : "BCV"}):
                    </span>
                    <strong style={{ fontSize: "15px", color: "#1864ab" }}>
                      Bs.{" "}
                      {(
                        (parseFloat(renewAmount) || 0) * (bcvRate || 1)
                ...
                {renewPaymentMethod !== "cash_usd" && (
                  <div className="form-group">
                    <label>
                      {renewPaymentMethod === "pending"
                        ? "Nota sobre el compromiso de pago"
              ...
              >
                ...
                >
                  {renewProcessing
                    ? "Registrando..."
                    : "Confirmar Cobro y Renovar 30 Días"}
                </button>
              </div>
            </form>
          </div>
