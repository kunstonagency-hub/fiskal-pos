...
export default function DeliveryDashboard({ storeId, isOnline, bcvRate }) {
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '100%', boxSizing: 'border-box', overflowX: 'hidden', margin: '0 auto', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px', gap: '12px' }}>
      ...
      {orders.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '64px 20px', background: '#fff', borderRadius: '16px', border: '1px dashed #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 320px), 1fr))', gap: '16px', width: '100%', boxSizing: 'border-box' }}>
          {orders.map((order) => {
            ...
            return (
              <div key={order.id} style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)', display: 'flex', flexDirection: 'column', width: '100%', boxSizing: 'border-box' }}>
                <div style={{ padding: '16px', borderBottom: '1px solid #e2e8f0', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '8px', background: '#f8fafc', boxSizing: 'border-box' }}>
                ...
                <div style={{ padding: '16px', minHeight: '120px', flex: 1, boxSizing: 'border-box' }}>
                  <div style={{ marginBottom: '16px', paddingBottom: '12px', borderBottom: '1px dashed #cbd5e1', fontSize: '13px', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
                    <div style={{ flex: '1 1 120px', minWidth: 0 }}>
                      <div style={{ fontWeight: 'bold', color: '#0f172a', marginBottom: '4px', wordBreak: 'break-word' }}>{order.customer_info?.nombre} {order.customer_info?.apellido}</div>
                      <div style={{ color: '#64748b', wordBreak: 'break-word' }}>Tel: {order.customer_info?.telefono}</div>
                    </div>
                    <button onClick={() => handleWhatsAppContact(order)} style={{ background: '#25D366', color: '#fff', border: 'none', padding: '6px 10px', borderRadius: '8px', fontSize: '12px', fontWeight: 'bold', cursor: 'pointer', flexShrink: 0 }}>WhatsApp</button>
                  </div>

                  {(parsedItems || []).map((item, idx) => (
                    <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px', fontSize: '14px', gap: '8px', flexWrap: 'nowrap' }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <span style={{ fontWeight: 'bold', color: '#10b981', marginRight: '6px' }}>{item.quantity || 1}x</span>
                        <span style={{ color: '#334155', fontWeight: 'bold', wordBreak: 'break-word' }}>{item.name}</span>
                        {item.customization && <span style={{ display: 'block', fontSize: '11px', color: '#d97706', marginTop: '4px', wordBreak: 'break-word' }}>📌 {item.customization}</span>}
                      </div>
                      <span style={{ fontWeight: '500', color: '#0f172a', flexShrink: 0 }}>${Number(item.price * (item.quantity || 1)).toFixed(2)}</span>
                    </div>
                  ))}
                </div>

                <div style={{ padding: '16px', background: '#f8fafc', borderTop: '1px solid #e2e8f0', borderBottom: '1px solid #e2e8f0', boxSizing: 'border-box' }}>
                  <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', gap: '8px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: '#475569', flexWrap: 'wrap' }}>
                    <CreditCard size={14} flexShrink={0} />
                    <span style={{ textTransform: 'capitalize', fontWeight: 'bold', wordBreak: 'break-word' }}>{order.payment_method?.replace('_', ' ') || 'No definido'}</span>
                  </div>
                </div>

                <div style={{ padding: '16px', display: 'flex', flexWrap: 'wrap', gap: '8px', boxSizing: 'border-box' }}>
                  {order.status === 'Pendiente' && (
                    <>
                      <button onClick={() => updateOrderStatus(order.id, 'Rechazado')} style={{ flex: '1 1 80px', background: '#fee2e2', color: '#ef4444', border: 'none', padding: '10px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>Rechazar</button>
                      <button onClick={() => updateOrderStatus(order.id, 'Preparando')} style={{ flex: '2 1 140px', background: '#10b981', color: '#fff', border: 'none', padding: '10px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>Aceptar y Cocinar</button>
                    </>
                  )}
                  {['Preparando', 'en_comercio'].includes(order.status) && (
                    <div style={{ width: '100%', textAlign: 'center', padding: '12px', background: '#f1f5f9', color: '#64748b', borderRadius: '8px', fontWeight: 'bold', fontSize: '13px', boxSizing: 'border-box' }}>
